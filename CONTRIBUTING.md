...
3. Every fixed rule ID in `src/ste100/rule_ids.py` is exercised at least once by something in `tests/corpus_dirty/` (two documented exceptions are noted in the harness itself: `STE-CSV-0009` aliases to `STE-BUD-0003`, and `STE-BUD-0004` is exercised by a direct `Engine` call instead of a fixture, because it's keyed to exact real project paths).
...
- **Fixed rules** (`src/ste100/rule_ids.py`): T4 (pronoun/comparative ambiguity), T5 (non-atomic-sentence structure: multiple `shall`, `and/or`, punctuation density, EARS conformance), the `S7-*` structural checks (bare numbers, articles, passive voice, `tbd`, abbreviations, terminology, `must` vs `shall`), CSV-integrity checks (`STE-CSV-*`), and word/sentence/paragraph budgets (`STE-BUD-*`). These are small, closed enumerations, hand-assigned directly in the source.
- **Bulk word/phrase lists** (`src/ste100/data/*.json`): T1 substitutions (424 entries as of this writing -- "use a plainer word"), T2 vague/unfalsifiable terms, T3 hedges (escape clauses, open-ended clauses, optionality phrases, superfluous infinitives), and T6 filler/intensifiers/weasel words/corporate speak/nominalizations/AI-tell phrases. Each entry carries a stable `STE-<test>-<CATEGORY>-<seq4>` ID (see `the active preset (src/ste100/presets/*.json)`'s `rule_id_taxonomy`), assigned by sorting the word list and numbering from 1 so a rebuild from the same input is byte-identical.

`ste_lint.py` loads `src/ste100/data/*.json` straight off disk at `src/ste100/paths.py:LINT_DATA_DIR` (the repo root's `src/ste100/data/`, not something generated at install time) -- so for day-to-day use, those JSON files **are** the rule data; you don't need to regenerate them to change what the linter catches.

**Known gap, please read before relying on it:** `devtools/build_lint_data.py` (which is supposed to regenerate `src/ste100/data/*.json` from a source word-list file) and `src/ste100/data/budgets.json`'s whole-file-budget targets still reference paths from the original Veistra monorepo this tool was extracted from (`handoff/prose_lint_wordlists.json`, `core/00-READ-FIRST.md`, `core/writing-standard.md`, `truths.csv`, `decisions-*.csv`, `timeline.csv`, `terminology.csv`) that don't exist in this standalone repo. Running `python -X utf8 devtools/build_lint_data.py` here fails with `FileNotFoundError` today. Until someone fixes that, **edit the `src/ste100/data/*.json` files by hand** for word-list changes (see below) rather than trying to run the generator.
...
1. Open `src/ste100/data/<the right file>.json` and add your entry to the appropriate array (for example a T1 substitution needs `pattern`, `suggestion`, `alts`, and a `source`; see the existing entries for the shape).
...
**New check logic** (new Python in `src/ste100/checks_*.py` or `src/ste100/engine.py`) -- higher bar. Open an issue first (use the "New rule / word-list addition" issue template) describing the ASD-STE100 rule, or other cited source (NASA SEH, MIL-STD-961E, INCOSE, etc. -- see the `source` fields already in `src/ste100/data/*.json` and `severity_overrides` in `the active preset (src/ste100/presets/*.json)`), that the check implements, plus example sentences it should and shouldn't flag. New fixed rule IDs go in `src/ste100/rule_ids.py` next to the existing `T4_*`/`T5_*`/`S7_*` constants, with an `EXPLAIN_TEXT` entry.
