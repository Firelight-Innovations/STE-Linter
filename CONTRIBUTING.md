...
3. Every fixed rule ID in `lint/rule_ids.py` is exercised at least once by something in `tests/corpus_dirty/` (two documented exceptions are noted in the harness itself: `VEI-CSV-0009` aliases to `VEI-BUD-0003`, and `VEI-BUD-0004` is exercised by a direct `Engine` call instead of a fixture, because it's keyed to exact real project paths).
